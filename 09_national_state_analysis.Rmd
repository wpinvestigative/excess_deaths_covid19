...
nat <- read_csv("data/outputs/national_and_state_summary_082420.csv") %>% 
  filter(state=="US.agg") %>% 
...
states <- read_csv("data/outputs/national_and_state_summary_082420.csv") %>% 
...
higher <- march %>% 
  filter(flag30=="Higher than baseline range") %>% 
  pull(state)

within <- march %>% 
  filter(flag30=="Within range") %>% 
  pull(state)

lower <- march %>% 
  filter(flag30=="Lower than baseline range") %>% 
  pull(state)
...
# change flag here maybe
    filter(flag30=="Higher than baseline range" & flag31=="Higher than baseline range") %>% 
...
other_states <- other_states %>%   
  select(Place=state, `All deaths`=all_cause_deaths,
         ...
         `Covid-19 % of excess`=percent_covid_in_excess, `Week 30`=flag30, `Week 31`=flag31) 
```

There were around 213,000 excess deaths between March and Aug 2, about 42 percent more than were publicly attributed to covid-19 at the time.
...
march <- march %>%   
  ...
  select(Place=state, `All deaths`=all_cause_deaths,
         `Excess deaths`=excess_deaths, `Covid-19 deaths`=covid_deaths,
#         `Covid-19 % of excess`=percent_covid_in_excess, `Week 23`=flag23, `Week 24`=flag30, lag, order=restriction_type) 
         `Covid-19 % of excess`=percent_covid_in_excess, `Week 30`=flag30, `Week 31`=flag31, lag) 
