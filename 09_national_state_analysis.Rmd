...
nat <- read_csv("data/outputs/national_and_state_summary_070320.csv") %>% 
  filter(state=="US.agg") %>% 
...
states <- read_csv("data/outputs/national_and_state_summary_070320.csv") %>% 
...
# change flag here maybe
    filter(flag24=="Higher than baseline range" & flag25=="Higher than baseline range") %>% 
...
other_states <- other_states %>%   
  select(Place=state, `All deaths`=all_cause_deaths,
         ...
         `Covid-19 % of excess`=percent_covid_in_excess, `Week 24`=flag24, `Week 25`=flag25) 
```

There were around 138,000 excess deaths between March and June, about 22 percent more than were publicly attributed to covid-19 at the time.
...
More recent data suggests that the gap between unattributed and attributed covid-19 deaths is shrinking. But growing in some states, like Arizona.

## Deaths through June 26, 2020
...
#         `Covid-19 % of excess`=percent_covid_in_excess, `Week 23`=flag23, `Week 24`=flag24, lag, order=restriction_type) 
         `Covid-19 % of excess`=percent_covid_in_excess, `Week 24`=flag24, `Week 25`=flag25, lag) 
